"""
Script ETL para cargar la tabla de hechos de pagos en el Data Warehouse.

Este script extrae información de pagos de la base de datos transaccional,
obtiene los datos asociados de plan de suscripción y carga los registros
en la tabla de hechos de pagos del Data Warehouse.
"""
...
def extraer_pagos_por_fecha(db_transacciones, fecha_transaccion):
    """
    Extrae información de pagos posteriores a una fecha específica desde la base transaccional.
    ...
    try:
        ...
        pagos = response.data
        logger.info(f"Extraídos {len(pagos)} pagos nuevos desde la base transaccional")
        return pagos
...
def main():
    ...
    """
    nombre_etl = "ETL_CARGAR_HECHOS_PAGOS"
